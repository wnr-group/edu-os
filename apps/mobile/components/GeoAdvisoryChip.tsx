...
const MUTED_ACCENT = "#6B7280";

export function GeoAdvisoryChip({ advisory }: { advisory: Advisory | null }) {
  if (!advisory || advisory.status === "neutral") {
    return (
      <View
        style={{ flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: MUTED_BG, borderRadius: 14, padding: 12 }}
        accessible
        accessibilityLabel="Location unavailable. GPS fix not acquired."
        accessibilityRole="image"
      >
        <View style={{ width: 32, height: 32, borderRadius: 10, backgroundColor: "#FFFFFF", alignItems: "center", justifyContent: "center" }}>
          <Ionicons name="location-outline" size={17} color={MUTED_ACCENT} />
        </View>
        <View style={{ flex: 1 }}>
          <Text style={{ fontSize: 13.5, fontFamily: "Inter_700Bold", color: MUTED_INK }}>Location unavailable</Text>
          <Text style={{ fontSize: 11.5, fontFamily: "Inter_400Regular", color: MUTED_INK, opacity: 0.75, marginTop: 1 }}>
            GPS fix not acquired · Attendance will submit as normal
          </Text>
        </View>
        <View style={{ backgroundColor: "#FFFFFF", borderRadius: 100, paddingHorizontal: 9, paddingVertical: 4 }}>
          <Text style={{ fontSize: 10, fontFamily: "Inter_700Bold", color: MUTED_INK, letterSpacing: 0.3 }}>NO GPS</Text>
        </View>
      </View>
    );
  }
  ...
  const title = isInside ? `On Campus (Approx) · ${advisory.geofenceName}` : `Off Campus (Approx) · ${distance} away`;
